...
public class ChatTool extends JFrame {
    ...
    String AIAvatar = "D:\\CODE\\DuAnTotNghiep\\CoachMeAI\\src\\main\\java\\com\\mycompany\\Image\\logo_chatbot.png ";
                ...
                if (matcher.matches()) {
                    ...
                    if (scheduleResponse.getMessage() != null && !scheduleResponse.getMessage().trim().isEmpty()) {
                        SwingUtilities.invokeLater(() -> {
                            appendMessage("", scheduleResponse.getMessage(), Color.white, AIAvatar);
                            chatArea.revalidate();
                            chatArea.repaint();
                        });
                    }
                    ...
                    SwingUtilities.invokeLater(() -> {
                        appendMessage("", normalResponse, Color.white, AIAvatar);
                        chatArea.revalidate();
                        chatArea.repaint();
                    });
                }
...
// Hàm kiểm tra thời gian có phải là trong tương lai hay không
    ...
    private void askForTestCreation(JSONObject taskData) {
        ...
        if (response == JOptionPane.YES_OPTION) {
            ...
            SwingUtilities.invokeLater(() -> appendMessage("", confirmationMessage, Color.white, AIAvatar));
    ...
    public void generateTestContent(String subject, int numberOfQuestions) {
        ...
        try {
            ...
            SwingUtilities.invokeLater(() -> {
                ...
                appendMessage("", successMessage, Color.white, AIAvatar);
            });

        } catch (Exception e) {
            e.printStackTrace();
            SwingUtilities.invokeLater(() -> {
                appendMessage("", "⚠️ Lỗi trong quá trình tạo câu hỏi hoặc quá trình tạo đã bị gián đoạn.", Color.white, AIAvatar);
            });
        }
    }
